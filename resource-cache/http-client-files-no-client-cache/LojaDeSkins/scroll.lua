...
function dxCreateScrollBar (identify, x, y, width, height, size, colors, values, postGUI)
    ...
    if not scrollbar.elements[identify] then
        ...
        if v.using then
            ...
            v.anim = {v.positions.offset, v.positions.offset, getTickCount()}
        else
            if not v.using and v.anim[2] ~= v.positions.offset then
                v.anim[2] = v.positions.offset
                v.anim[3] = getTickCount()
            end

            offset = interpolateBetween(v.anim[1], 0, 0, v.anim[2], 0, 0, (getTickCount() - v.anim[3]) / 1000, 'Linear') 
            v.anim[1] = offset    
        end
        
        dxDrawRectangle (x, y, w, h, tocolor (unpack (colors.background)), postGUI)
        dxDrawRectangle (x, (v.using and v.positions.offset or offset), w, v.positions.size, (v.using and tocolor (unpack (colors.using)) or tocolor (unpack (colors.scroll))), postGUI)
    end
end
