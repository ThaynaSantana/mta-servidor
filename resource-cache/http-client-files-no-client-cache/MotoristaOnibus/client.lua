...
addEvent("AirNew>JOBUS>AbrirPainel<Client", true)
addEventHandler("AirNew>JOBUS>AbrirPainel<Client", localPlayer, Window)
...
function botoes(bt,state)
	if state == "up" and bt == "left" then
		...
		if source == escolher then
			...
			triggerServerEvent("AirNew>request>Startrota>Server>MotoristaDeOnibus",localPlayer,localPlayer,nome)	
			if isElement(myWindow) then
				destroyElement(myWindow)
				showCursor(false)
			end			
...
addEvent("AirNew>JOBUS>blips<Client", true)
addEventHandler("AirNew>JOBUS>blips<Client", localPlayer, blips)