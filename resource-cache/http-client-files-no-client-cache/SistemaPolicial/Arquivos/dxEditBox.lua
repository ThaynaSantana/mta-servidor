...
function dxDrawEditBox(text, x, y, w, h, password, maxCharacter, element)
	local getText = getElementData(element, "text6") or setElementData(element, "text6", "")
	...
	dxDrawRectangle(x, y, w, h, tocolor(0, 0, 0, 160), false)
	if (#getElementData(element, "text6") == 0) then
		dxDrawText(text, x + 5, y, x + w - 10, y + h, tocolor(255, 255, 255, 150), 1, font, "center", "center", true, false, false)
	end
	if (dxGetTextWidth(password and string.gsub(getElementData(element, "text6"), ".", "•") or getElementData(element, "text6"), 1, font) <= w - 10) then
		dxDrawText(password and string.gsub(getElementData(element, "text6"), ".", "•") or getElementData(element, "text6"), x + 5, y, x + w - 10, y + h, tocolor(255, 255, 255, 255), 1, font, "left", "center", true, false, false)
	else
		dxDrawText(password and string.gsub(getElementData(element, "text6"), ".", "•") or getElementData(element, "text6"), x + 5, y, x + w - 10, y + h, tocolor(255, 255, 255, 255), 1, font, "right", "center", true, false, false)
	end
	if (getElementData(element, "state") == true) then
		if (dxGetTextWidth(password and string.gsub(getElementData(element, "text6"), ".", "•") or getElementData(element, "text6"), 1, font) <= w - 10) then
			dxDrawLine(x + 5 + dxGetTextWidth(password and string.gsub(getElementData(element, "text6"), ".", "•") or getElementData(element, "text6"), 1, font), y + 5, x + 5 + dxGetTextWidth(password and string.gsub(getElementData(element, "text6"), ".", "•") or getElementData(element, "text6"), 1, font), y + h - 5, tocolor(255, 255, 255, math.abs(math.sin(getTickCount() / 255) * 255)), 1, false)
		else
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("dxEditBox6")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("dxEditBox6")[editBoxId])) then
				if (getElementData(getElementsByType("dxEditBox6")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("dxEditBox6")[editBoxId], "state", true)
				end
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("dxEditBox6")) do
		if (getElementData(element, "state") == true) then
			if (#getElementData(element, "text6") < getElementData(element, "maximum")) then
				setElementData(element, "text6", getElementData(element, "text6")..button)
			end
		end
	end
end
...
function dxKeyElement(button, press)
	...
	if (press) and (button == "backspace") then
		for _, element in ipairs(getElementsByType("dxEditBox6")) do
			if (getElementData(element, "state") == true) then
				if (#getElementData(element, "text6") > 0) then
					setElementData(element, "text6", string.sub(getElementData(element, "text6"), 1, #getElementData(element, "text6") - 1))
				end
			end
		end
	end
