...
function Gerenciador_Corp()
    ...
    dxDrawText("Armamento GROTA", x*411, y*238, x*901, y*273, tocolor(255, 255, 255, 255), 1.60, "default", "center", "center", false, false, false, false, false)
    ...
    if button=="left" and state=="down" then
        if exibir == true then
            if isMouseInPosition(x*569, y*293, x*175, y*30) then
                triggerServerEvent("GROTA:GiveWeapons01", localPlayer)
            end
        end
    end
end)
    ...
    if button=="left" and state=="down" then
        if exibir == true then
            if isMouseInPosition(x*569, y*333, x*175, y*30) then
                triggerServerEvent("GROTA:GiveWeapons02", localPlayer)
            end
        end
    end
end)
    ...
    if button=="left" and state=="down" then
        if exibir == true then
            if isMouseInPosition(x*569, y*373, x*175, y*30) then
                triggerServerEvent("GROTA:GiveWeapons03", localPlayer)
            end
        end
    end
end)
    ...
    if button=="left" and state=="down" then
        if exibir == true then
            if isMouseInPosition(x*569, y*413, x*175, y*30) then
                triggerServerEvent("coleteGROTA", localPlayer)
            end
        end
    end
end)
...
-- Abrir Painel
addEvent("GROTA:AbrirMarkerPOL", true)
addEventHandler("GROTA:AbrirMarkerPOL", getRootElement(), function()
