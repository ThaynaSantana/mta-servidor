...
addEvent("AirNew>JOBEntregadorDeEtanol>AbrirPainel<Client", true)
addEventHandler("AirNew>JOBEntregadorDeEtanol>AbrirPainel<Client", localPlayer, Window)
...
function botoes(bt,state)
	if state == "up" and bt == "left" then
		...
		if source == escolher then
			...
			triggerServerEvent("AirNew>request>Startrota>Server>EntregadorDeEtanol",localPlayer,localPlayer,nome)	
			if isElement(myWindow) then
				destroyElement(myWindow)
				showCursor(false)
			end			
...
addEvent("AirNew>JOBEntregadorDeEtanol>blips<Client", true)
addEventHandler("AirNew>JOBEntregadorDeEtanol>blips<Client", localPlayer, blips)
