...
addEvent("AirNew>JOBEntregadorDeMaconha>AbrirPainel<Client", true)
addEventHandler("AirNew>JOBEntregadorDeMaconha>AbrirPainel<Client", localPlayer, Window)
...
function botoes(bt,state)
	if state == "up" and bt == "left" then
		...
		if source == escolher then
			...
			triggerServerEvent("AirNew>request>Startrota>Server>EntregadorDeMaconha",localPlayer,localPlayer,nome)	
			if isElement(myWindow) then
				destroyElement(myWindow)
				showCursor(false)
			end			
...
addEvent("AirNew>JOBEntregadorDeMaconha>blips<Client", true)
addEventHandler("AirNew>JOBEntregadorDeMaconha>blips<Client", localPlayer, blips)
