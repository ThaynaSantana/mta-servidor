...
addEvent("AirNew>JOBEntregadorDePescador>AbrirPainel<Client", true)
addEventHandler("AirNew>JOBEntregadorDePescador>AbrirPainel<Client", localPlayer, Window)
...
function botoes(bt,state)
	if state == "up" and bt == "left" then
		...
		if source == escolher then
			...
			if nome == nil then outputChatBox("Selecione Uma Rota",255,255,255,true) return end
			triggerServerEvent("AirNew>request>Startrota>Server>EntregadorDePescador",localPlayer,localPlayer,nome)	
			if isElement(myWindow) then
				destroyElement(myWindow)
				showCursor(false)
			end			
...
addEvent("AirNew>JOBEntregadorDePescador>blips<Client", true)
addEventHandler("AirNew>JOBEntregadorDePescador>blips<Client", localPlayer, blips)
