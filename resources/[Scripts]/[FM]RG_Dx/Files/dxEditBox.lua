...
function dxDrawEditBox(text, x, y, w, h, password, maxCharacter, element)
	...
	dxDrawRectangle(x, y, w, h, tocolor(238, 233, 233, 80), false)
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("dxEditBox3")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("dxEditBox3")[editBoxId])) then
				if (getElementData(getElementsByType("dxEditBox3")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("dxEditBox3")[editBoxId], "state", true)
				end
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("dxEditBox3")) do
		if (getElementData(element, "state") == true) then
