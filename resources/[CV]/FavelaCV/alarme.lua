...
RestricLocation["location1"] = {1531.231, -699.415, -104.467} 
RestricLocation["location2"] = {1456.078, -657.416, 200.467}
TeleportLocation = {1492.752, -765.073, 87.041}
GroupName = "CV" 
GroupNameBy = 2 
MsgInvasao = "Você não tem permissão para entrar nessa base!"
...
function sendMsgOwners( PlayerID )
	...
	for i, aPlayer in ipairs(connectedPlayers) do
		if(PlayerHaveLevel (aPlayer) == true) then
			sendMsg(aPlayer," O Jogador " ..getPlayerName ( PlayerID ) .." #FF0000está tentando entrar em sua base!")
		end
	end
end
